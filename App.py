...
def w_is_url(string):
    ...
    result = [result.append(f'<a href="{i}" style="color:blue;" target="blank">{i}</a>') for i in text if i in b] #else result.append(i)]
...
def home():
    ...
    if request.method == 'POST':
        ...
        if request.form.get("request") == "create_folder_request" and MODE == "admin":file_d_path = request.form.get("path").strip("/");dirname = request.form.get("dirname");os.mkdir(f"/{file_d_path}/{dirname}")
        if request.form.get("request") == "delete_request" and MODE == "admin":
            ...
            if os.path.isfile(file_d_path):shutil.copyfile(f"/{file_d_path}", f"Trash/{file_copy[-1]}");os.remove(f"/{file_d_path}")
            ...
            return redirect(request.form.get("path"))
    return render_template("index.html" , files_data=files_data , back_button_path="/" , current_path=request.path , usermode=MODE , files_num=len(files_data))
...
def error404(e):
        ...
        try:
            try:
                file_c = sorted(os.listdir(f"{folder_path}/{myurl}"))
                ...
                return render_template("index.html" , files_data=files_data , back_button_path=back_button_path , current_path=request.path , usermode=MODE , files_num=len(files_data))
            except:
                if file_type(f"{folder_path}/{myurl}") == 'txt':return send_file(f"{folder_path}/{myurl}")
                else:return send_file(f"{folder_path}/{myurl}")
